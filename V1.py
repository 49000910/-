...
class UltimateMiniGuard:
    ...
    def pop_preview_window(self):
        try: 
            ...
            sns = sorted(list(set(s.strip() for s in data.split('\n') if s.strip())))
        except: 
            return
        ...
        def start_btn():
            sns_list = list(lb.get(0, tk.END))
            self.execute_auto(sns_list)
            pv.destroy()
            
        tk.Button(pv, text="启动录入", bg="#4caf50", fg="white", command=start_btn).pack(fill=tk.X)
    ...
    def _auto_run(self, sns):
        ...
        try:
            ...
            mid_delay = float(self.spin_mid.get())
            ...
            for sn in sns:
                ...
                # 2. 写入剪贴板 (拆分 Lambda)
                def update_clip(x=sn):
                    self.root.clipboard_clear()
                    self.root.clipboard_append(x)
                self.root.after(0, update_clip)
                time.sleep(e1) 
                ...
                self.root.after(0, lambda x=sn: self.add_log(x, "auto"))
        except: 
            pass
